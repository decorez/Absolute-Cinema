                ...
                <tr class="hover:bg-white/5 transition">
                    ...
                    <td class="px-6 py-4 text-[#D2C1B6] text-xs space-y-0.5">
                        ...
                        @if($booking->schedule)
                            ...
                            <p class="font-semibold text-white">
                                {{ optional($booking->schedule->studio)->name }}
                            </p>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex flex-wrap gap-1 max-w-[180px]">
                            ...
                            @empty
                                <span class="text-[#D2C1B6] text-xs">
                                    -
                                </span>
                            @endforelse
                        ...
                        <div class="flex justify-center gap-2">
                            ...
                            @if($booking->status === 'pending')
                                <form action="{{ route('admin.bookings.approve', $booking->id) }}" method="POST" class="m-0">
                                    @csrf
                                    <button type="submit"
                                            class="rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 text-xs font-bold text-emerald-400 hover:bg-emerald-500 hover:text-white transition">
                                        Approve
                                    </button>
                                </form>
                            @endif
                            ...
                            @if($booking->status !== 'cancelled')
                                <form action="{{ route('bookings.destroy', $booking->id) }}"
                                    ...
                                    <button type="submit"
                                            class="rounded-xl bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 text-xs font-bold text-amber-400 hover:bg-amber-500 hover:text-white transition">
                                        Cancel
                                    </button>
                                </form>
                            @endif
                            ...
                            <form action="{{ route('admin.bookings.forceDelete', $booking->id) }}"
                                  method="POST"
                                  class="delete-permanent-form m-0">
                                @csrf
                                @method('DELETE')

                                <button type="submit"
                                        class="rounded-xl bg-red-500/10 border border-red-500/20 px-3 py-1.5 text-xs font-bold text-red-400 hover:bg-red-500 hover:text-white transition">
                                    Delete
                                </button>
                            </form>
